...
#else
	(ret=CloseHandle(fd)) || warnE("CloseHandle");
#endif
	fd=BADFD;
	_current_baudrate=0;
	return ret;
...
//		debug_print_buf (len);
		...
		if(got==0) { /*warn("ReadFile: timeout expires with %d bytes unread",len);*/ return false; }
...
inline bool SerialPort::_write (const uchar *data, int len) const {
#ifdef LINUX
...
//	debug_print_buf (data,len);
	if(WriteFile (fd, data, len, &written, 0) && len==written)
	{
		return true;
	}
