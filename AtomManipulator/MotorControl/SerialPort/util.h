...
#if defined(USE_WINMM) or defined(LINUX)
...
//
...
#ifdef LINUX
	#define CC0		"\033[0m"
	#define CC1		"\033[0;44;37m"
	#define CC2i	"\033[1;40;37m"
	#define CC2w	"\033[1;40;31m"
	#define CC2e	"\033[1;40;31m"
	#define CC3		"\033[0;40;37m"
	#define CC4		"\033[1;40;33m"
	...
	#define info(fmt, args...)	( _printf(CC1"[%s]"CC3" %-33.33s "CC2i" INFO: "CC4 fmt CC0"\n", time_str3(), __PRETTY_FUNCTION__, ## args) )
	#define infoe(fmt, args...)	( _printf(CC1"[%s]"CC3" %-33.33s "CC2i" INFO: "CC4 fmt ": %s"CC0"\n", time_str3(), __PRETTY_FUNCTION__, ## args, strerror(errno)) )
	#define infoE(fmt, args...)	( _printf(CC1"[%s]"CC3" %-33.33s "CC2i" INFO: "CC4 fmt ": %s"CC0"\n", time_str3(), __PRETTY_FUNCTION__, ## args, StrError()) )
	#define warn(fmt, args...)	( _printf(CC1"[%s]"CC3" %-33.33s "CC2w" WARN: "CC4 fmt CC0"\n", time_str3(), __PRETTY_FUNCTION__, ## args) )
	#define warne(fmt, args...)	( _printf(CC1"[%s]"CC3" %-33.33s "CC2w" WARN: "CC4 fmt ": %s"CC0"\n", time_str3(), __PRETTY_FUNCTION__, ## args, strerror(errno)) )
	#define warnE(fmt, args...)	( _printf(CC1"[%s]"CC3" %-33.33s "CC2w" WARN: "CC4 fmt ": %s"CC0"\n", time_str3(), __PRETTY_FUNCTION__, ## args, StrError()) )
	#define die(fmt, args...)	( _die (printf(CC1"[%s]"CC2e"  ERROR "CC3"(in %s, line %d, %s): "CC4 fmt CC0"\n", time_str3(), __FILE__, __LINE__, __PRETTY_FUNCTION__, ## args)) )
	#define diee(fmt, args...)	( _die (printf(CC1"[%s]"CC2e"  ERROR "CC3"(in %s, line %d, %s): "CC4 fmt ": %s"CC0"\n", time_str3(), __FILE__, __LINE__, __PRETTY_FUNCTION__, ## args, strerror(errno))) )
	#define dieE(fmt, args...)	( _die (printf(CC1"[%s]"CC2e"  ERROR "CC3"(in %s, line %d, %s): "CC4 fmt ": %s"CC0"\n", time_str3(), __FILE__, __LINE__, __PRETTY_FUNCTION__, ## args, StrError())) )
#else
	inline static int _die(int n) { exit(-1); return n; }
	#define _ooxx_(ss,SS)	inline static bool ss(const char *fmt, ...) {           \
								va_list ap;                           \
								va_start(ap,fmt);                     \
